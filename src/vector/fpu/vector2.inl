...
template<class T>
constexpr void vector<T, 2, isa::fpu>::zero() noexcept
{
    x = y = T(0);
}

template<class T>
constexpr bool vector<T, 2, isa::fpu>::isZero() const noexcept
{
    return (T(0) == x) & (T(0) == y);
}

template<class T>
constexpr bool vector<T, 2, isa::fpu>::isNan() const noexcept
{
    return math::isnan(x) || math::isnan(y);
}

template<class T>
constexpr bool vector<T, 2, isa::fpu>::isEqual(const vector& v, T eps) const noexcept
{
    return math::equal(x, v.x, eps) & math::equal(y, v.y, eps);
}

template<class T>
constexpr bool vector<T, 2, isa::fpu>::isUnit(T eps) const noexcept
{
    return equal(squareLength(), T(1), eps);
}

template<class T>
constexpr void vector<T, 2, isa::fpu>::abs() noexcept
{
    x = math::abs(x);
    y = math::abs(y);
}
...
template<class T>
constexpr void vector<T, 2, isa::fpu>::round() noexcept
{
    x = math::round(x);
    y = math::round(y);
}
...
template<class T>
constexpr void vector<T, 2, isa::fpu>::clamp(T lo, T hi) noexcept
{
...
template<class T>
constexpr void vector<T, 2, isa::fpu>::saturate() noexcept
{
    clamp(T(0), T(1));
}

template<class T>
constexpr T vector<T, 2, isa::fpu>::sum() const noexcept
{
    return x + y;
}

template<class T>
constexpr T vector<T, 2, isa::fpu>::sumAbs() const noexcept
{
...
template<class T>
constexpr void vector<T, 2, isa::fpu>::select(const vector& v, const boolvector& c) noexcept
{
    x = c.x ? v.x : x;
    y = c.y ? v.y : y;
}
...
template<class T>
constexpr void vector<T, 2, isa::fpu>::store(T *p) const noexcept
{
    p[0] = x;
    p[1] = y;
}
